...
def alerts_latest(
    ...
    def _build_payload():
        ...
        if strength:
            normalized_strength = strength.strip().upper()
            query = query.filter(Alert.strength == normalized_strength)
        if category:
    ...
    return cached_json_response(
        request,
        cache_key=cache_key,
        ttl_seconds=settings.CACHE_TTL_ALERTS_LATEST_SECONDS,
...
def alert_history(
    ...
    return cached_json_response(
        request,
        cache_key=cache_key,
        ttl_seconds=settings.CACHE_TTL_ALERT_HISTORY_SECONDS,
        fetch_fn=_build_payload,
        private=True,
    )
...
def copilot_recommendations(
    ...
    return cached_json_response(
        request,
        cache_key=cache_key,
        ttl_seconds=settings.CACHE_TTL_COPILOT_FEED_SECONDS,
        fetch_fn=_build_payload,
        private=True,
    )
...
def alerts_summary(
    ...
    def _build_payload():
        ...
        counts = {alert_type: int(count) for alert_type, count in rows}
        return {"since": since.isoformat(), "counts": counts}

    return cached_json_response(
        request,
        cache_key=cache_key,
        ttl_seconds=settings.CACHE_TTL_ALERT_SUMMARY_SECONDS,
        fetch_fn=_build_payload,
        private=True,
    )
...
def alerts_last_digest(request: Request, api_key=Depends(rate_limit)):
    ...
    return cached_json_response(
        request,
        cache_key=cache_key,
        ttl_seconds=settings.CACHE_TTL_ALERT_LAST_DIGEST_SECONDS,
        fetch_fn=_build_payload,
        private=True,
    )
...
def copilot_runs(
    ...
    return cached_json_response(
        request,
        cache_key=cache_key,
        ttl_seconds=settings.CACHE_TTL_COPILOT_RUNS_SECONDS,
        fetch_fn=_build_payload,
        private=True,
    )
